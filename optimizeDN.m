function [U1,U2]=optimizeDN(G1,G2,G3,D1,ini)
% trace掉环境的指标；
TT1=contracttensors(conj(G1),4,[1,2],G1,4,[1,2]);
...
% 左右的环境乘进矩阵里,and svd；
G1=contracttensors(tl1,2,2,G1,4,3);
G1=contracttensors(G1,4,4,tr1,2,2);
G1=permute(G1,[2,1,3,4]);
G1=normalize(G1);
[V1,l,V2]=psvd(G1,D1);
V1=contracttensors(V1,3,3,sqrt(l),2,1);
V2=contracttensors(sqrt(l),2,2,V2,3,1);
% 调整回原来的tensor
U1=contracttensors(tl1inv,2,2,V1,3,2);
U1=permute(U1,[1,3,2]);
U1=normalize(U1);
U2=contracttensors(V2,3,3,tr1inv,2,2);
U2=permute(U2,[1,3,2]);
U2=normalize(U2);
end